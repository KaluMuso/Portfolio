    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          <Search size={15} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-600" />
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by name, company, email or district..."
            className="w-full bg-white dark:bg-[#0d0d14] border border-gray-200 dark:border-white/[0.08] rounded-xl pl-11 pr-4 py-3 text-sm text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-blue-500/40 transition-colors"
          />
        </div>
      ...
      {/* Table */}
      <div className="bg-white dark:bg-[#0d0d14] border border-gray-200 dark:border-white/[0.06] rounded-2xl overflow-hidden">
        {filtered.length === 0 ? (
          <div className="py-20 text-center">
            <Users size={36} className="mx-auto mb-3 text-gray-300 dark:text-gray-700" />
          ...
          <>
                ...
                <thead>
                  <tr className="border-b border-gray-200 dark:border-white/[0.06]">
                    {["Applicant", "Industry", "Contact", "District", "Joined"].map((h) => (
                      <th key={h} className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-[0.15em] text-gray-500 dark:text-gray-600">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.04]">
                  {filtered.map((entry) => (
                    <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-gray-800 dark:text-gray-200">{entry.name}</p>
                            <p className="text-xs text-gray-500">{entry.company_name}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold bg-gray-100 dark:bg-white/[0.05] text-gray-600 dark:text-gray-400 px-2.5 py-1 rounded-lg">
                          {entry.industry}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{entry.district}</td>
                      <td className="px-6 py-4 text-xs text-gray-500 dark:text-gray-600">
            ...
            {/* Mobile cards */}
            <div className="lg:hidden divide-y divide-gray-100 dark:divide-white/[0.04]">
              {filtered.map((entry) => (
                <div key={entry.id} className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    <span className="bg-gray-100 dark:bg-white/[0.05] text-gray-600 dark:text-gray-400 px-2 py-1 rounded-lg">{entry.industry}</span>
            ...
            <div className="px-6 py-3 border-t border-white/[0.06] text-xs text-gray-500 dark:text-gray-600 font-bold">
