...
export default function AdminSettingsPage() {
  ...
  const save = async () => {
    ...
    try {
      const supabase = createBrowserClient();
      const { error: sbErr } = await supabase.from("site_settings").upsert({ id: 1, ...settings });
    ...
    return (
      <AdminShell title="Settings">
        <div className="space-y-4">
          {[...Array(4)].map((_, i) => <div key={i} className="h-24 bg-[#0d0d14] rounded-2xl animate-pulse" />)}
        </div>
      </AdminShell>
    );
  }
  ...
  return (
    <AdminShell title="Settings" subtitle="Site configuration & contact info">
      <div className="max-w-2xl space-y-6">
        ...
        <div className="bg-[#0d0d14] border border-white/[0.06] rounded-2xl p-6 space-y-5">
          ...
          {[
            ...
            { key: "contact_email",   label: "Contact Email",    icon: Mail,  placeholder: "vergeosales@gmail.com", type: "email" },
